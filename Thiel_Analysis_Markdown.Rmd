...
Transfer$key = factor(Transfer$key, levels = c("Pre", "Post"))
...
There is a significant effect on transference (*t* = 5.13, *p* < 0.001). 
...
There is a significant effect on confidence (*t* = 12.5, *p* < 0.001). 
...
There is a significant effect on preparedness (*t* = 11.83, *p* < 0.001).


```{r, echo=FALSE}
Survaydt = fread("Thiel cadaver pelvic exam Survey Analysis.csv")
colnames(Survaydt)
new_colnames = c('Exp.Live.Patient', 'Exp.')
setnames()
