...
#9 I feel more confident performing a PE ona  live patient - Seems to be the most important outcome. Regress to find predicting variables? 
...
Type.dt[type.change,]$Type = 'Task.Trainer'
type.change = Type.dt$Type == '3'
Type.dt[type.change,]$Type = 'Neither'
Type.dt$Type = factor(Type.dt$Type, levels = c('Thiel', 'Task.Trainer', 'Neither'))
#merge group and type to make one group col.
Type.dt$Group = with(Type.dt, interaction(Group, Type))

# Means
  means = data.table(aggregate(Score ~ Group , Type.dt, mean))
  means$Score = round(means$Score, digits = 2)
# n of each group
  means$N = aggregate(Score ~ Group, Type.dt, length)$Score
####################################################################################################################################
#t.test
...
TT.Results = t.test(Type.dt[Group == 'Pre' & Type == 'Task.Trainer']$Score, Type.dt[Group == 'Post' & Type == 'Task.Trainer']$Score, paired = TRUE)
#view results
TT.Results 
